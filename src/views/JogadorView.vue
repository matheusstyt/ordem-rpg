<template>
    <div class="container-geral">
        ...
        <div class="item pessoal-personagem">
            <Avatar :vida="vida" :sanidade="sanidade" :ocultismo="ocutismo" :esforco="esforco" :movimento="movimento" :reacao="reacao" :acao="acao" @vidaU="vidaU" @sanidadeU="sanidadeU" @ocultismoU="ocultismoU" @esforcoU="esforcoU"/>
        </div>
        <div class="item pessoal-detalhes">
            <form @submit="salvarDetalhes($event)" class="formDetalhes" action="">
                ...
                <div  >
                    ...
                    <div>
                        <input class="input-padrao0" spellcheck="false" type="text" name="nome" id="id-nome" v-model="nome">
                    </div>
                </div>
                <div>
                    <label for="jogador">Jogador</label>
                    <div>
                        <input class="input-padrao0" spellcheck="false" type="text" name="jogador" id="id-jogador" v-model="jogador">
                    </div>
                </div>
                <div  >
                    <label for="ocupacao">Origem</label>
                    <div>
                        <input class="input-padrao0" spellcheck="false" type="text" name="origem" id="id-origem" v-model="origem">
                    </div>
                </div>
                <div class="line-padrao">
                    <label for="idade">Idade</label>
                    <div>
                        <input class="input-padrao0" spellcheck="false" type="number" name="idade" id="id-idade" v-model="idade">
                    </div>
                ...
                <div>
                    ...
                    <div>
                        <input class="input-padrao0" spellcheck="false" type="text" name="sexo" id="id-sexo" v-model="sexo">
                    </div>
                ...
                <div>
                    ...
                    <div>
                        <input class="input-padrao0" spellcheck="false" type="text" name="lnascimento" id="id-lnascimento" v-model="lnascimento">
                    </div>
                ...
                <div>
                    ...
                    <div>
                        <input class="input-padrao0" spellcheck="false" type="text" name="lresidencia" id="id-lresidencia" v-model="lresidencia">
                    </div>
        ...
        <button class="btn-delete" @click="deleteP">DELETAR PERSONAGEM</button>
        ...
        <Antescendentes :antescendentes="antescendentes" @antescedentesU="antescedentesU" :display="displayAntescendentes" @updateDisplayA="updateDisplayA"/>
        <Inventario :armamentos="armamentos" :peso="peso" :display="displayInventario" @updateDisplayI="updateDisplayI"/>
    </div>
...
export default{
    components: { Rituais, Atributos, Detalhes, Avatar, Resistencias, Pericias, DadoP, Antescendentes, Inventario},
    data(){
        return{
            displayInventario: 'display: none;',
            displayAntescendentes: 'display: none;',
            ...
            movimento :   0,
            peso : 0,
            vida : {id: 0, atual: 8, maximo: 0},
            sanidade : {id: 1, atual: 0, maximo: 0},
            ocutismo : {id: 2, atual: 0, maximo: 0},
            esforco :  {id: 3, atual: 0, maximo: 0},
            esforcoA : 5,
            esforcoM : 9,
            periciaN: '',
            periciaV: '',
        ...
        updateDisplayI(display){this.displayInventario = display},
        updateDisplayA(display){this.displayAntescendentes = display},
        displayModalU(displayM){this.displayModal = displayM},
        vidaU(vida){this.vida = vida},
        sanidadeU(sanidade){this.sanidade = sanidade},
        ocultismoU(ocultismo){this.ocultismo = ocultismo},
        esforcoU(esforco){this.esforco = esforco},
        antescedentesU(antescendentes){this.antescendentes = antescendentes; console.log(antescendentes)},
        async deleteP(){
            console.log(`porra`)
            const data = JSON.stringify({
                idVida: this.idVida,
                idSanidade: this.idSanidade,
                idOcultismo: this.idOcultismo,
        ...
        abrirInventario(){
            this.displayInventario = 'display:block;'
        },
        abrirAntescendentes(){
            this.displayAntescendentes = 'display:block;'
        ...
        async updateDados(){
            const data = JSON.stringify(
                {
                    ...
                    jogador: this.jogador.toUpperCase(),
                    origem: this.origem.toUpperCase(),
                    idade: this.idade,
                    sexo: this.sexo.toUpperCase(),
                    lnascimento: this.lnascimento.toUpperCase(),
                    lresidencia: this.lresidencia.toUpperCase(),
                    atual_v: this.vida.atual,
                    atual_s: this.sanidade.atual,
                    atual_e: this.esforco.atual,
                    //atual_o: this.ocultismo.atual,
                    maximo_v: this.vida.maximo,
                    maximo_s: this.sanidade.maximo,
                    maximo_e: this.vida.maximo,
                    //maximo_o: this.vida.maximo,
                    ...
                    idEsforco: this.idEsforco,
                    idPersonagem: this.idPersonagem,
                    idAntescendentes: this.idAntescendentes,
                    idArmas: this.idArmas,
            ...
            console.log(data)
            const req = await fetch(`http://localhost:5000/updateperson`,{
                method: "POST",
                headers: { "Content-Type": "application/json"},
                body: data
            });
        ...
        try {
          let dataJson = JSON.parse(this.$route.params.dJson)
            this.idSessao = dataJson.idSessao,
            this.idVida = dataJson.idVida,
            this.idSanidade = dataJson.idSanidade,
            this.idOcultismo = dataJson.idOcultismo,
            this.idEsforco = dataJson.idEsforco,
            this.idPersonagem = dataJson.idPersonagem,
            this.idAntescendentes = dataJson.idAntescendentes,
            this.idArma = dataJson.idArma,
            ...
            this.antescendentes = dataJson.antescendentes
            this.armamentos = dataJson.armamentos
            console.log(this.datJson)   
        } catch (error) {
            this.$router.push({name:`mestre`})
        }
       
    },watch:{
        nome(newValue){this.updateDados()},
        jogador(newValue){this.updateDados()},
        origem(newValue){this.updateDados()},
        idade(newValue){this.updateDados()},
        sexo(newValue){this.updateDados()},
        lnascimento(newValue){this.updateDados()},
        lresidencia(newValue){this.updateDados()},

         vida(newValue){this.updateDados(); console.log(newValue)},
         sanidade(newValue){this.updateDados()},
         esforco(newValue){this.updateDados()},

        // antescendentes(newValue){this.updateA(); console.log('watch',newValue)}
...
.btn-delete{
    background-color: rgba(78, 11, 9, 0.7);
    margin: 0 auto;
    margin-bottom: 20px;  
}
.btn-delete:hover{
    background-color: rgba(216, 41, 35, 0.7);
