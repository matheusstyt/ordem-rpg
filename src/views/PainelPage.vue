    ...
    <div class="content-right" v-if="system == false">
      <div class="content-sessions-list">
        <div class="content-session-add flex" v-if="list_sessions.length === 0">
          ...
          <button @click="open_modal_session">+</button>
        </div>
        <div class="content-session-add" v-else>
        ...
        >
          <ul>
            <li
              v-for="(session, index) in list_sessions"
              :key="session.id"
      ...
      <div class="content-matches-list">
        ...
        <div class="content-session-open" style="display: block">
          <ul>
            <li
              v-for="(match, index) in list_matches"
              :key="match.id"
        ...
        <ul>
          <li v-for="(item, index) in list_pendente">
            ...
            <div class="pendente-container-btn">
              <button
                id="contact-okay"
                @click="aceitar_pedido(item.fk_origem, item.fk_destino, item.id)"
              ></button>
              <button id="contact-delete" @click="excluir_pedido(item.id)">
                Excluir
              </button>
            </div>
          </li>
        ...
        <ul>
          <li v-for="(item, index) in list_partidas">
            ...
            <div class="pendente-container-btn">
              ...
              <button
                id="contact-delete"
                @click="excluir_pedido_partida(item.id)"
              ></button>
            </div>
          </li>
        </ul>
...
export default {
  ...
  methods: {
    ...
    async get_session() {
      ...
      axios
        .get(url_session, { params: { fk_mestre: this.user_id }, headers: headers })
        .then((res) => {
          console.log(res.data);
    ...
    async get_contact() {
      ...
      axios
        .get(url, {
          params: { fk_user: sessionStorage.getItem("token") },
          headers: headers,
        })
        .then((res) => {
          console.log(res.data);
...
@media screen and (max-width: 600px) {
  // estilo para telas menores btn okay-pendentes
  button {
    border: 1px solid black;
  }
  #contact-okay {
    padding: 0.7em;
    background-color: rgb(81, 236, 133);
  }
  #contact-okay::before {
    content: "";
  }
  #contact-okay::after {
    ...
    width: 1em;
    height: 1em;
    background-image: url("@/assets/ico/check_ico.svg");
  ...
  // estilo para telas menores btn delete-pendentes

  #contact-delete {
    background-color: rgba(233, 73, 73, 0.7);
    padding: 0.7em;
  }
  #contact-delete::before {
    content: "";
  ...
  #contact-delete::after {
    ...
    width: 1em;
    height: 1em;
    background-image: url("@/assets/ico/cancel_ico.svg");
    background-repeat: no-repeat;
    background-size: cover;
    position: absolute;
    transform: translate(-50%, -50%);
