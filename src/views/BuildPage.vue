...
export default {
  ...
  data() {
    return {
      ...
      pericias: [
        { id: 1, treino: "DESTREINADA", tipo: "treinada", nome: "ATLETISMO", valor: 0 },
  ...
  methods: {
    ...
    u_nascimento(nascimento) {
      this.nascimento = nascimento;
    },
    u_residencia(residencia) {
    ...
    verificar_detalhes() {
      if (
        this.nome.length > 5 &&
        this.origem.length > 5 &&
        this.classe.length > 5 &&
        this.NEX >= 0 &&
        this.trilha.length >= 0 &&
        this.patente.length > 5 &&
        this.idade > 3 &&
        this.nascimento.length > 4 &&
        this.residencia.length > 4
      ) {
        alert("detalhes");
      }
    },
